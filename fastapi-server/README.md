...
Для наполнения системы тестовыми данными используется скрипт `generate_test_data.py`. Он создает:

1. Пользователей различных ролей (студенты, преподаватели, администраторы)
2. Учебные группы и дисциплины
3. Оценки студентов по различным дисциплинам

### Использование скрипта

```bash
# Генерация всех данных с очисткой базы
python generate_test_data.py --clear

# Генерация с указанием количества пользователей
python generate_test_data.py --count 200

# Генерация только оценок (не затрагивает существующих пользователей)
python generate_test_data.py --grades-only
```

### Тестовые учетные данные

После запуска скрипта будут созданы следующие пользователи:
